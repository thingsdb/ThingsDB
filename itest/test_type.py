...
class TestType(TestBase):
    ...
    async def _TODO_test_wrap_only(self, client):
    ...
    async def _TODO_test_mod_arr(self, client):
    ...
    async def _TODO_test_set_prop(self, client):
        await client.query(r'''
            set_type('Pet', {});
    ...
    async def _TODO_test_method(self, client):
        await client.query(r'''
            set_type('Person', {
                name: 'str',
    ...
    async def _TODO_test_new_type(self, client0):
    ...
    async def _TODO_test_mod_type_add(self, client0):
        await client0.query(r'''
    ...
    async def _TODO_test_enum_wrap_thing(self, client0):
        await client0.query(r'''
            set_type('TColor', {
                name: 'str',
                code: 'str'
    ...
    async def _TODO_test_enum_wrap_int(self, client0):
        await client0.query(r'''
            set_type('TColor', {
                name: 'str',
                code: 'str'
    ...
    async def _TODO_test_wrap_other(self, client0):
    ...
    async def _TODO_test_circular_dep(self, client):
        await client.query(r'''
            new_type('Tic');
            new_type('Tac');
            new_type('Toe');
    ...
    async def _TODO_test_mod_del(self, client):
        await client.query(r'''
            new_type('Foo');
            set_type('Foo', {
                foo: 'Foo?',
    ...
    async def _TODO_test_mod(self, client):
        await client.query(r'''
            set_type('Person', {
                name: 'str',
                age: 'uint',
        ...
        with self.assertRaisesRegex(
                ValueError,
                r'function `mod_type` expects argument 2 to be '
                r'`all`, `add`, `del`, `hid`, `mod`, `rel`, `ren` or `wpo` '
                r'but got `x` instead'):
    ...
    async def _TODO_test_mod_type(self, client):
        await client.query(r'''
            new_type('Tic');
            new_type('Tac');
            new_type('Toe');
    ...
    async def _TODO_test_type_mismatch(self, client):
        await client.query(r'''
            new_type('Tic');
            new_type('Tac');
            new_type('Toe');
    ...
    async def _TODO_test_migrate(self, client0):
    ...
    async def _TODO_test_sync_add(self, client0):
        await client0.query(r'''
            set_type('Book', {
                title: 'str',
            });
    ...
    async def _TODO_test_circular_del(self, client):
        await client.query(r'''
            new_type('Foo');
            new_type('Bar');
    ...
    async def _TODO_test_del_type(self, client):
        await client.query(r'''
            new_type('Tic');
            new_type('Tac');
            new_type('Toe');
    ...
    async def _TODO_test_type_definitions(self, client):
        await client.query(r'''
    ...
    async def _TODO_test_mod_type_add_closure_and_ts(self, client0):
        now = int(time.time())
    ...
    async def _TODO_test_mod_type_mod_closure(self, client0):
        await client0.query(r'''
            set_type('Chat', {
                name: 'str',
                messages: '[str]'
    ...
    async def _TODO_test_mod_type_mod_advanced0(self, client0):
        with self.assertRaisesRegex(
                OperationError,
    ...
    async def _TODO_test_mod_type_mod_advanced1(self, client0):
        await client0.query(r'''
            set_type('Chat', {
                messages: '[str]'
            });
    ...
    async def _TODO_test_mod_type_mod_advanced2(self, client0):
        with self.assertRaisesRegex(
                OperationError,
    ...
    async def _TODO_test_wpo(self, client0):
        await client0.query(r'''
            new_type('A', true);
            new_type('B', false);
            mod_type('B', 'wpo', true);
    ...
    async def _TODO_test_err_handling(self, client0):
        with self.assertRaisesRegex(
                TypeError,
    ...
    async def _TODO_test_init_create(self, client0):
        await client0.query(r'''
            set_type('X', {
                a: new_type('A'),
                b: new_type('B'),
    ...
    async def _TODO_test_rename(self, client0):
        await client0.query(r'''
            set_type('A', {i: 'int'});
            set_type('B', {a: 'A'});
        ''')
    ...
    async def _TODO_test_rename_prop(self, client0):
        await client0.query(r'''
            set_type('Test', {
                arr: '[]',
                set: '{}',
    ...
    async def _TODO_test_wrap(self, client0):
        await client0.query(r'''
    ...
    async def _TODO_test_same_deep(self, client):
    ...
    async def _TODO_test_type_name_id(self, client):
        with self.assertRaisesRegex(
                LookupError,
                r'multiple Id \(\'#\'\) definitions on type `User`'):
            res = await client.query(r"""//ti
                set_type('User', {
                    id: '#',
                    key: '#',
        ...
        with self.assertRaisesRegex(
            ...
            await client.query(r"""//ti
                mod_type('User', 'add', 'key', '#');
            """)

        with self.assertRaisesRegex(
                TypeError,
                r'cannot convert a property into an Id \(\'#\'\) definition'):
            await client.query(r"""//ti
                mod_type('Person', 'mod', 'name', '#');
            """)

        with self.assertRaisesRegex(
                TypeError,
                r'cannot modify an Id \(\'#\'\) definition'):
            await client.query(r"""//ti
                mod_type('User', 'mod', 'id', 'str');
            """)

        with self.assertRaisesRegex(
                NumArgumentsError,
                r'function `mod_type` with task `add` takes at '
                r'most 4 arguments when adding an Id \(\'#\'\) definition;'):
            await client.query(r"""//ti
                mod_type('Person', 'add', 'id', '#', ||nil);
            """)

        with self.assertRaisesRegex(
                LookupError,
                r'type `User` has no property `id`'):
            await client.query(r"""//ti
                mod_type('User', 'rel', 'id', 'id');
            """)
        ...
        with self.assertRaisesRegex(
            ...
            mod_type('R', 'rel', 'user', 'id');
            """)

        await client.query(r"""//ti
            mod_type('User', 'ren', 'id', 'key');
            mod_type('Person', 'add', 'id', '#');
        """)
        ...
        for cl in (client, client1):
            self.assertEqual(
                await cl.query(r'.pw'),
                {"key": person_id, "name": "Cato"})
            self.assertEqual(
                await cl.query(r'.uw'),
                {"id": user_id, "name": "Iris"})

        await client.query(r"""//ti
            mod_type('Person', 'del', 'id');
        """)
    ...
    async def _TODO_test_prefix_flags(self, client):
        with self.assertRaisesRegex(
                ValueError,
    ...
    async def _TODO_test_rename_type_with_flags(self, client):
        res = await client.query(r"""//ti
            new_type('A');
            set_type('B', {
                a0: 'A',
    ...
    async def _TODO_test_email(self, client):
        with self.assertRaisesRegex(
                ValueError,
    ...
    async def _TODO_test_url(self, client):
        with self.assertRaisesRegex(
                ValueError,
                r'invalid declaration for `def` on type `E`; '
    ...
    async def _TODO_test_tel(self, client):
        with self.assertRaisesRegex(
                ValueError,
                r'invalid declaration for `def` on type `E`; '
