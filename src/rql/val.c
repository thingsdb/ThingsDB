...
int rql_val_to_file(rql_val_t * val, FILE * f)
{
    ...
    case RQL_VAL_PRIMITIVES:
        if (qp_fadd_type(f, QP_ARRAY_OPEN)) return -1;
        for (vec_each(val->via.elems_, rql_val_t, v))
        {
            if (rql_val_to_file(v, f)) return -1;
        }
        return qp_fadd_type(f, QP_ARRAY_CLOSE);
    case RQL_VAL_ELEM:
        return qp_fadd_int64(f, (int64_t) val->via.elem_->id);
    case RQL_VAL_ELEMS:
        if (qp_fadd_type(f, QP_ARRAY_OPEN)) return -1;
        for (vec_each(val->via.elems_, rql_elem_t, elem))
        {
