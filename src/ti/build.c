...
int ti_build_setup(uint8_t this_id, uint8_t from_id, ti_stream_t * stream)
{
    ...
    ti_pkg_t * pkg = ti_pkg_new(0, TI_PROTO_NODE_REQ_SETUP, NULL, 0);
    if (!pkg)
    {
    ...
    build->this_id = this_id;
    build->from_id = from_id;
...
static void build__on_setup_cb(ti_req_t * req, ex_enum status)
{
    ...
    ti()->archive->sevid = &ti()->node->sevid;
