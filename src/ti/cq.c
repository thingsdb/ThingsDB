...
static int cq__arrow(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    query->rval = ti_arrow_from_node(nd);
...
static int cq__chain_name(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    val = name ? ti_thing_weak_get_val(thing, name) : NULL;
...
static int cq__f_del(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    ti_name_t * name;
    ti_raw_t * rname;
    ti_thing_t * thing = (ti_thing_t *) query_weak_get_val(query);
    _Bool from_rval = thing == query->rval;
...
static int cq__f_endswith(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    ti_val_t * val = query_weak_get_val(query);
    ...
    ti_val_drop(query->rval);
    query->rval = ti_vbool_get(ti_raw_endswith(val, query->rval));
...
static int cq__f_filter(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    ti_val_t * iterval = query_weak_get_val(query);
    ti_arrow_t * arrow;
    ...
    case TI_VAL_THING:
    {
        ...
        for (vec_each(((ti_thing_t *) iterval)->props, ti_prop_t, p))
        {
            ...
            {
                ti_incref(p->name);
                if (ti_thing_set(thing, p->name, &p->val))
                    goto failed;
            }
...
static int cq__f_startswith(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    ti_val_drop(query->rval);
    query->rval = ti_vbool_get(ti_raw_startswith(val, query->rval));
...
static int cq__f_t(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    for (n = 0; child; child = child->next->next)
    {
        ...
        {
            ex_set(e, EX_BAD_DATA,
                    "function `t` only accepts `int` arguments, but "
...
static int cq__primitives(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    case CLERI_GID_T_FALSE:
        query->rval = ti_val_get_false();
        break;
    case CLERI_GID_T_FLOAT:
        if (!node->data)
        {
            node->data = ti_val_create_float(strx_to_double(node->str));
            if (!node->data)
            {
                ex_set_alloc(e);
                return e->nr;
    ...
    case CLERI_GID_T_INT:
        if (!node->data)
        {
            node->data = ti_val_create_int(strx_to_int64(node->str));
            if (!node->data)
            {
                ex_set_alloc(e);
                return e->nr;
    ...
    case CLERI_GID_T_REGEX:
        if (!node->data)
        {
            ...
            if (!regex)
                return e->nr;
            node->data = ti_val_create_regex(regex);
            if (!node->data)
            {
                ti_regex_drop(regex);
                ex_set_alloc(e);
    ...
    case CLERI_GID_T_STRING:
        if (!node->data)
        {
            ...
            node->data = ti_val_create_raw(raw);
            if (!node->data)
            {
                ti_raw_drop(raw);
                ex_set_alloc(e);
...
static int cq__scope_name(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    if (!val && name)
        val = ti_thing_weak_get_val(query->target->root, name);
...
static int cq__scope_thing(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    for (; child; child = child->next->next)
    {
        ...
        if (ti_thing_set(name, query->rval))
        {
            ti_name_drop(name);
            goto alloc_err;
        }
    ...
    query->rval = ti_val_create_thing(thing);
    if (!query->rval)
        goto alloc_err;
...
static int cq__tmp_assign(ti_query_t * query, cleri_node_t * nd, ex_t * e)
{
    ...
    assert (name_nd->len >= 2);
    assert (ti_name_is_valid_strn(name_nd->str + 1, name_nd->len - 1));
    ...
    else
    {
        ...
        assert (assign_nd->len == 2);
        assert (assign_nd->str[1] == '=');
        prop = query_get_tmp_prop(query, name);
        if (!prop)
        {
            ex_set(e, EX_INDEX_ERROR,
                    "`%.*s` is undefined", (int) name->n, name->str);
    ...
    if (prop)
    {
        ...
        ti_val_weak_copy(&prop->val, query->rval);
    }
    else
    {
        prop = ti_prop_weak_createv(name, query->rval);
        if (!prop)
        {
            goto alloc_err;
        }
...
alloc_err_with_prop:
    free(prop);  /* name is dropped, value too */
