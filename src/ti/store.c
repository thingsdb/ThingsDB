...
int ti_store_store(void)
{
    ...
    if (!ti_stat_fn || !users_fn || !access_fn || !dbs_fn ||
        ti_store_status_store(ti_stat_fn) ||
        ti_names_store(names_fn) ||
        ti_users_store(users_fn) ||
        ti_store_access_store(thingsdb->access, access_fn) ||
        ti_dbs_store(dbs_fn))
        goto stop;
    ...
    for (vec_each(thingsdb->dbs, ti_db_t, db))
    {
        ...
        names_fn = fx_path_join(db_path, ti__store_names_fn);
        ...
        if (!access_fn || !names_fn || !things_fn || !db_fn ||
            ti_store_access_store(db->access, access_fn) ||
...
int ti_store_restore(void)
{
    ...
    if (    !users_fn || !ti_stat_fn || !access_fn || !dbs_fn ||
            ti_store_status_restore(ti_stat_fn) ||
            ti_users_restore(users_fn) ||
            ti_store_access_restore(&thingsdb->access, access_fn) ||
    ...
    for (vec_each(thingsdb->dbs, ti_db_t, db))
    {
        ...
        if (!access_fn || !names_fn || !things_fn || !db_fn ||
            !(namesmap = ti_names_restore(names_fn)) ||
            ti_store_access_restore(&db->access, access_fn) ||
            ti_things_restore(db->things, things_fn) ||
            ti_db_restore(db, db_fn) ||
            ti_things_restore_skeleton(db->things, namesmap, skeleton_fn) ||
            ti_things_restore_data(db->things, namesmap, data_fn))
            goto stop;
    }
