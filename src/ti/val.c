...
static ti_val_t * val__unp_map(ti_vup_t * vup, size_t sz, ex_t * e)
{
    ...
    case TI_KIND_C_WANO:
    {
        ...
        if (sz != 1 ||
            mp_next(vup->up, &mp_val) != MP_ARR || mp_val.via.sz != 2 ||
            mp_next(vup->up, &mp_spec_raw) != MP_BIN)
        {
            ex_set(e, EX_BAD_DATA,
                "wano type must be written according the "
                "following syntax: {\""TI_KIND_S_WANO"\": [spec_raw, {...}]");
            return NULL;
        }
        ...
        {
            ex_set(e, EX_BAD_DATA,
                "wano type is expecting a wrapped `"TI_VAL_THING_S"` but "
                "got type `%s` instead",
                ti_val_str(vthing));
            ti_val_unsafe_drop(vthing);
            return NULL;
        ...
        wano = ti_wano_create((ti_thing_t *) vthing, ano);
        if (!wano)
            ex_set_mem(e);
        ti_val_unsafe_drop(vthing);
        ti_val_unsafe_drop((ti_val_t *) ano);
        return (ti_val_t *) wano;
    }
...
int ti_val_init_common(void)
{
    ...
    /* names */
    val__async_name = (ti_val_t *) ti_names_from_str_slow("async");
    val__data_name = (ti_val_t *) ti_names_from_str_slow("data");
    val__time_name = (ti_val_t *) ti_names_from_str_slow("time");
    val__year_name = (ti_val_t *) ti_names_from_str_slow("year");
    val__month_name = (ti_val_t *) ti_names_from_str_slow("month");
    val__day_name = (ti_val_t *) ti_names_from_str_slow("day");
    val__hour_name = (ti_val_t *) ti_names_from_str_slow("hour");
    val__minute_name = (ti_val_t *) ti_names_from_str_slow("minute");
    val__second_name = (ti_val_t *) ti_names_from_str_slow("second");
    val__gmt_offset_name = (ti_val_t *) ti_names_from_str_slow("gmt_offset");
    val__module_name = (ti_val_t *) ti_names_from_str_slow("module");
    val__deep_name = (ti_val_t *) ti_names_from_str_slow("deep");
    val__flags_name = (ti_val_t *) ti_names_from_str_slow("flags");
    val__load_name = (ti_val_t *) ti_names_from_str_slow("load");
    val__beautify_name = (ti_val_t *) ti_names_from_str_slow("beautify");
    val__parent_name = (ti_val_t *) ti_names_from_str_slow("parent");
    ...
    val__anonymous_name = (ti_val_t *) ti_names_from_str_slow("<anonymous>");
...
size_t ti_val_get_len(ti_val_t * val)
{
    ...
    case TI_VAL_ANO:
        return (
            ((ti_ano_t *) val)->type->fields->n +
            ((ti_ano_t *) val)->type->methods->n +
...
int ti_val_gen_ids(ti_val_t * val)
{
    ...
    case TI_VAL_ANO:
        break;
    case TI_VAL_WANO:
        if (!((ti_wano_t *) val)->thing->id)
            return ti_thing_gen_id(((ti_wano_t *) val)->thing);
        /*
         * New things 'under' an existing thing will get their own task,
         * so here we do not need recursion.
         */
