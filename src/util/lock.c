 ...
 *                      path locked if a lock file exists in the path.
 */
...
const char * lock_str(lock_t rc)
{
    ...
    case LOCK_IS_LOCKED_ERR:
        return "path is locked by another process";
    ...
    case LOCK_REMOVED:
        return "path is successfully unlocked";
    case LOCK_NEW:
        return "path is successfully locked with a new lock file";
    case LOCK_OVERWRITE:
        return
            "path is successfully locked but a lock file existed which "
            "indicates that the process was not closed correctly last time";
    }
    return "unknown type";
}
...
static lock_t LOCK_create(const char * lock_fn, pid_t pid)
{
    lock_t rc_success = (!fx_file_exist(lock_fn)) ?
