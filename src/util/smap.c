...
{
    smap_t * smap = calloc(1, sizeof(smap_t));
    if (!smap)
        return NULL;
...
void smap_destroy(smap_t * smap, smap_destroy_cb cb)
{
    ...
    if (smap->nodes)
    {
        for (uint_fast16_t i = 0, end = smap->sz << SMAP_BSH; i < end; i++)
        {
            if ((*smap->nodes)[i])
            {
...
void ** smap_getaddr(smap_t * smap, const char * key)
{
    ...
    while (nd && !strncmp(nd->key, ++key, nd->n))
    {
        ...
        nd = (*nd->nodes)[k - (nd->offset << SMAP_BSH)];
    }
...
void * smap_popn(smap_t * smap, const char * key, size_t n)
{
    ...
    data = smap__popn(NULL, nd, key + 1, n - 1);
    if (data && !--smap->n)
    {
        free(smap->nodes);
        smap->nodes = NULL;
...
int smap_items(smap_t * smap, char * buf, smap_item_cb cb, void * arg)
{
    ...
    if (smap->data && (rc = (*cb)(buf, 0, smap->data, arg)))
        return rc;
    ...
    for (uint_fast16_t i = 0, end = smap->sz << SMAP_BSH; i < end; i++)
    {
        if (!(nd = (*smap->nodes)[i]))
            continue;
        *buf = (char) (i + (smap->offset << SMAP_BSH));
...
int smap_values(smap_t * smap, smap_val_cb cb, void * arg)
{
    ...
    if (smap->data && (rc = (*cb)(smap->data, arg)))
        return rc;
...
static size_t smap__longest_key_size(smap_node_t * node)
{
    ...
    if (node->nodes)
    {
        for (uint_fast16_t i = 0, end = node->sz << SMAP_BSH; i < end; i++)
        {
            if (!(nd = (*node->nodes)[i]))
                continue;
            n = smap__longest_key_size(nd);
        ...
        void * arg)
{
    ...
    if (node->nodes)
    {
        ...
        for (uint_fast16_t i = 0, end = node->sz << SMAP_BSH; i < end; i++)
        {
            if (!(nd = (*node->nodes)[i]))
                continue;
            *(buf + n) = (char) (i + (node->offset << SMAP_BSH));
        ...
        void * data)
{
    ...
    if (n)
    {
        ...
        if (*nd)
            return smap__addn(*nd, key, n, data);
...
static void smap__merge_node(smap_node_t * node)
{
    ...
    for (i = 0, end = node->sz << SMAP_BSH; i < end; i++)
    {
        if ((*node->nodes)[i])
            break;
    }
        ...
        const char * key)
{
    ...
    if (node->nodes)
    {
        ...
        smap_node_t ** next = &(*node->nodes)[k - (node->offset << SMAP_BSH)];
        ...
        size_t n)
{
    ...
    if (node->nodes)
    {
        ...
        return (*next) ? smap__popn(node, next, key + 1, n - 1) : NULL;
    }
    return NULL;
}
