...
//static void alloc_cb(uv_handle_t *handle, size_t sugsz, uv_buf_t *buf)
//{
//    uv_tls_t * sclient = handle->data;
//    ti_stream_t * stream = sclient->stream;
//    LOGC("ALLOC0");
//    if (!stream->n && stream->sz != sugsz)
//    {
//        free(stream->buf);
//        stream->buf = malloc(sugsz);
//        memset(stream->buf, 0, sugsz);
//
//        if (stream->buf == NULL)
//        {
//            buf->base = NULL;
